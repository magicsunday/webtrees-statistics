...
# differ in punctuation → compile every PO to a MO. The stages are chained
# as a dependency LINE (compile → resolve-fuzzy → merge → extract), not as
# sibling prerequisites, so `make -j` (or an inherited parallel MAKEFLAGS)
# cannot run compile before merge or let the fuzzy resolver rewrite a PO
# while it is being compiled — a stale/corrupt MO despite a green exit.
lang: .logo lang-compile ## Extract POT, merge PO, auto-resolve fuzzy, compile MO (full i18n pipeline).
...
lang-merge: lang-extract ## Update each locale's PO from the latest POT (drops obsolete entries, seeds missing locales).
	@$(COMPOSE_RUN) sh -c 'set -eu; \
		command -v msgmerge >/dev/null 2>&1 || apk add --no-cache gettext >/dev/null 2>&1; \
		if [ -n "$(LOCALES)" ]; then \
			for loc in $(LOCALES); do \
				po="resources/lang/$$loc/messages.po"; \
				if [ ! -f "$$po" ]; then \
					mkdir -p "resources/lang/$$loc"; \
					msginit --no-translator --locale="$$loc" --input=$(POT_FILE) --output-file="$$po" >/dev/null; \
					echo "  + Seeded $$po from POT"; \
					echo "    note: for a region/script tag (e.g. en-US, zh-Hans) msginit leaves the"; \
					echo "    Plural-Forms placeholder — set nplurals/plural once by hand; msgmerge preserves it."; \
				else \
...
# no-op.
lang-resolve-fuzzy: lang-merge ## Auto-resolve fuzzy entries with trivial punctuation diffs in the new msgid.
	@$(COMPOSE_RUN) sh -c 'set -eu; \
...
lang-compile: lang-resolve-fuzzy ## Compile every messages.po to its sibling messages.mo.
	@$(COMPOSE_RUN) sh -c 'set -eu; \
		command -v msgfmt >/dev/null 2>&1 || apk add --no-cache gettext >/dev/null 2>&1; \
