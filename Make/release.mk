...
#   make release 1.0.0 NOTES_FILE=/tmp/notes.md
#   make release VERSION=1.0.0 NOTES_FILE=/tmp/notes.md   (also supported)
#
...
MODULE_NAME      := webtrees-statistics
...
# (see GitHub issues #194, #196 in webtrees-fan-chart). Must be unique per module.
SCOPE_NS         := StatisticsVendor
...
release-check:
	...
	@if [ -z "$(VERSION)" ]; then \
		...
		echo "   or:  make release VERSION=1.0.0"; \
		exit 1; \
	fi
	@if ! echo "$(VERSION)" | grep -qE '^[0-9]+\.[0-9]+\.[0-9]+$$'; then \
		echo "Error: VERSION must be semver (e.g. 1.0.0 — no v prefix, no suffix)"; \
		exit 1; \
	fi
...
## Example: make release 1.0.1 NOTES_FILE=/tmp/release-notes.md
## Example: make release 1.0.1 NOTES="Bug fix release"
##
