...
final readonly class MarriageMatrixRepository
{
     ...
     * and wife surnames come from the primary name row (`n_num = 0`) so each
     * spouse contributes exactly one surname per family. Pinning the primary
     * name — rather than merely excluding the `_MARNM` married-name type, or even
     * restricting to `n_type = 'NAME'` — keeps a wife under her birth surname and
     * prevents BOTH the alternate name forms webtrees stores as their own `name`
     * rows under the same `n_id` (`ROMN`/`FONE`/`_HEB` transliterations,
     * `_AKA`/`_AKAN` aliases — these carry a non-`NAME` `n_type`) AND a second
     * top-level `1 NAME` line (which carries `n_type = 'NAME'` with `n_num = 1`,
     * so an `n_type` filter alone would not collapse it) from fanning the
     * marriage out across several surnames and double-counting it.
     *
