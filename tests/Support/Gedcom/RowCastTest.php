 ...
 * loosely-typed `stdClass` rows Eloquent returns. The branches matter: SQLite +
 * MySQL disagree on whether an `INTEGER` column comes back as an `int` or a
 * numeric string, and a missing column (typo, schema drift) must surface as the
 * supplied default rather than a fatal undefined-property warning.
 *
 * DataProvider rows index the documented branches 1:1 so a future additional
 * branch (`is_int` shortcut, JSON-encoded fallback, …) forces a new yield
...
final class RowCastTest extends TestCase
{
     ...
     * "non-string value" row catches the case where a numeric column
     * accidentally appears on a string-typed read.
     *
    ...
    /**
     * Drives every documented branch of {@see RowCast::int()} against a rebuilt
     * stdClass row — happy path, numeric-string coercion, missing column with
     * both the implicit and an explicit default, and the non-numeric fallback.
     *
    ...
    /**
     * Drives every documented branch of {@see RowCast::string()}: happy path on
     * a string column, missing column with both defaults, and the
    ...
    /**
     * Build a stdClass mirror of an Eloquent row from the given property map —
     * an empty map models the absent-column scenario.
     *
     * @param array<string, mixed> $properties
     */
