...
- **`Support/Locale/`** — Locale-aware string lookups. Members: `CenturyName` (year → localised century label), `IsoCountryMap` (free-text country name or ISO code → ISO-3166-1 alpha-2, built on PHP intl across nine pre-seeded locales + manual aliases for common GEDCOM abbreviations; ISO-3166-1 alpha-3 codes such as `DEU` bridge through ICU's shared display name rather than a hand-kept table, and the UK home-nation codes `ENG`/`SCT`/`WLS`/`NIR` fold onto `GB`; the core `CountryService` is deliberately avoided — it is `@deprecated` for removal in webtrees 2.3; `resolveFromPlace()` peels the country segment off a full GEDCOM place string first), `SpelledNumber` (low-cardinal integer → localised word, e.g. `1 → "one"` / `1 → "ein"`), `ZodiacLabels` (canonical → locale-translated zodiac sign map), `HistoricalEventCatalog` (curated, place-correlated catalogue of broadly-documented excess-mortality events — wars, pandemics, famines — whose labels annotate the mortality-anomaly years under a single "coincides with" lead-in; `keysFor` is framework-free for unit-testing, `labelsFor` resolves the localised event labels), `DecadeName` (localised decade labels shared by every per-decade widget — mirrors `CenturyName`), `MonthName` (localised nominative month names + GEDCOM `JAN`..`DEC` code ordering shared by the by-month donuts and the period × month heatmap).
