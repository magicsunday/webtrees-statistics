...
- **`Templates/Names.phtml`** — Composes two Sections: FREQUENCIES (common surnames + male/female given-name tag-cloud cards), EVOLUTION (given-name popularity stream graph + surname × surname marriage `ChordDiagram` cards).
- **`Templates/TreeHealth.phtml`** — Composes three Sections: DOCUMENTATION (source-citation coverage `RateList` + average-generation-length `Scalar`), GAPS (missing-event gaps `RateList`), GROWTH (tree-growth `LineChart`).
...
- **`Partials/RecordsTable.phtml`** — Tree-records hall-of-fame table extracted from `Templates/Overview.phtml` so the Overview's CHRONICLE card body and any future per-individual records widget can share it.
- **`Partials/<Widget>.phtml`** — Thin shells that emit the `data-widget` JSON marker and the empty target element consumed by chart-lib widgets. Current set: `DonutChart`, `ProgressList`, `LineChart`, `BarChart`, `StackedBar`, `DivergingBar`, `AreaDensity`, `GeoMap`, `SankeyFlow`, `ChordDiagram`, `StreamGraph`, `TagCloud`, `RateList`, `Scalar`, `InfoTrigger` (the legacy chart-header info popover trigger — superseded by the Card `info` parameter on new code but still consumed by widget partials).
