...
- **`Statistic.php`** — `final readonly` aggregator service. Constructor-injects `StatisticsData` (core) plus the twenty-one repositories. Public methods return either scalars, typed DTOs from `Model/<Widget>/` (LineChart / StackedBar / Sankey / Chord / StreamGraph / Tree / Record / Ranking / Metric / HeroStats payloads), `[{label, value, class?}]` shapes for chart-lib widgets, `label => count` maps for the ProgressList partial, or `list<RankingEntry>` for the entity podiums (Top-N most descendants / largest families / oldest). `getHeroStats()` composes the six headline metrics the hero header renders into a `HeroStats` DTO.
