...
Each tab template under `resources/views/modules/statistics-chart/tabs/` composes its content from PHP fluent builders in `src/View/` — `Card::for($module, $title)`, `Section::create($title)`, `Widget::donutChart(...)`, `Component::scalar(...)` etc. — rendered into the three-folder kebab-case layout (`tabs/` + `widgets/` + `components/`). Widget partials under `widgets/` emit `data-widget` / `data-payload` / `data-options` JSON markers, and the shared JS dispatcher `WebtreesStatistic.renderWidgets(pane)` hands each off to the matching [`@magicsunday/webtrees-chart-lib`](https://github.com/magicsunday/webtrees-chart-lib) widget. The hero above the nav is driven by `components/hero.phtml`, which consumes a `HeroStats` DTO returned by `Statistic::getHeroStats()`. Illustrations are emitted via the `Illustration` enum (21 thematic SVG icons under `components/illustration.phtml`). Theme accents (Wine / Slate / Sage / Ochre / Rose / Deceased) are exposed via the `Accent` enum.
